...
export default function AdminPage() {
  ...
  const fetchData = useCallback(async (s: string) => {
    ...
    setLoading(false);
    if (!res.ok) { setAuthError('Contraseña incorrecta'); setAuthed(false); return; }
  ...
  const togglePlan = async (p: Proveedor) => {
    ...
    await fetch('/api/admin/plan', {
      ...
      body: JSON.stringify({ proveedorId: p.id, plan: newPlan, meses: 1 }),
    });
                    ...
                    <td style={{ padding: '14px 18px' }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                        <div style={{
                          ...
                          background: p.activo ? '#4CAF50' : '#666',
                          flexShrink: 0,
                        }}/>
                        <span style={{
                          fontFamily: 'var(--font-geist-sans), sans-serif',
