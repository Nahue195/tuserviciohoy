...
/* ─── Contact block (WhatsApp / Cotización) ──────────────────────────── */
function ContactBlock({ p, dark }: { p: ProveedorCard; dark?: boolean }) {
  const isCoti = p.modoContacto === 'COTIZACION';
  const waNum = p.whatsapp?.replace(/\D/g, '') ?? '';
  const msg = isCoti
    ? `Hola ${p.nombre}! Vi tu perfil en TuServicioHoy y quiero pedir un presupuesto.`
    : `Hola ${p.nombre}! Vi tu perfil en TuServicioHoy y quiero coordinar un servicio.`;
  const href = waNum
    ? `https://wa.me/${waNum}?text=${encodeURIComponent(msg)}`
    : `https://wa.me/?text=${encodeURIComponent(msg)}`;

  const bg = dark ? 'rgba(255,255,255,0.05)' : '#FFFBF3';
  const border = dark ? '1px solid rgba(255,255,255,0.08)' : '1px solid #EFE5D0';
  const labelColor = dark ? 'rgba(245,237,222,0.35)' : '#8B7D6B';
  const textColor = dark ? '#F5EDDE' : '#1A1208';

  return (
    <div style={{ background: bg, border, borderRadius: 16, padding: '20px' }}>
      <div style={{ fontFamily: 'var(--font-geist-sans), sans-serif', fontSize: 10, fontWeight: 700, letterSpacing: '0.8px', textTransform: 'uppercase', color: labelColor, marginBottom: 12 }}>
        {isCoti ? 'Pedí tu presupuesto' : 'Contactar'}
      </div>
      <p style={{ fontFamily: 'var(--font-geist-sans), sans-serif', fontSize: 13, color: labelColor, margin: '0 0 16px', lineHeight: 1.55 }}>
        {isCoti
          ? 'Este profesional trabaja por proyecto. Contale qué necesitás y te manda un presupuesto.'
          : 'Este profesional coordina por WhatsApp. Escribile para consultar disponibilidad y precios.'}
      </p>
      <a
        href={href}
        target="_blank"
      ...
      >
        ...
        {isCoti ? 'Pedir presupuesto' : 'Escribir por WhatsApp'}
      </a>
    </div>
  );
}
  ...
  return (
    <div className="min-h-full pb-[96px]" style={{ background: '#F5EDDE' }}>
      ...
      <div className="px-4 py-5 flex flex-col gap-6">
        ...
        <div>
          {p.modoContacto === 'TURNO' || !p.modoContacto ? (
            <>
